...
import { normalizeMeta } from "./serializers";
...
const stripSplat = winston.format((info) => {
  ...
  delete (info as any)[Symbol.for("splat")];
  return info;
});
...
const sanitize = winston.format((info) => {
  ...
  const normalized = normalizeMeta(rest as Record<string, unknown>);
 ...
 */
const devPrintf = printf(
  ({ level, message, timestamp: ts, service, ...meta }) => {
    const coloredLevel = (() => {
      switch (level) {
        case "fatal":
        case "error":
