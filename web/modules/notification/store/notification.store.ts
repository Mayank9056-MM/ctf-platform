// modules/notifications/lib/notification.store.ts
...
export const useNotificationStore = create<NotificationUIState>()(
  devtools(
    (set) => ({
      ...
      setPanelTab: (tab) =>
        set({ panelTab: tab }, false, "setPanelTab"),

      setInboxPage: (page) =>
        set({ inboxPage: page }, false, "setInboxPage"),
      ...
      optimisticallyDismiss: (id) =>
        set(
          (s) => ({
            optimisticDismissedIds: new Set([
              ...s.optimisticDismissedIds,
              id,
            ]),
          }),
          false,
